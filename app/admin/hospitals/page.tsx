...
import { AdminPublicUserList } from "@/components/AdminPublicUserList";
import { LogoutButton } from "@/components/LogoutButton";
import { getCurrentHospital } from "@/lib/auth";
import { listHospitals } from "@/lib/db";
import { listBlockedPublicUsers } from "@/lib/public-db";
...
export default async function AdminHospitalsPage(){
  ...
  const[hospitals,blockedUsers]=await Promise.all([listHospitals(),listBlockedPublicUsers()]);
  return <main className="auth-page min-h-screen px-4 py-8 sm:px-6"><div className="mx-auto max-w-6xl"><header className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[.2em] text-emerald-300"><ShieldCheck className="size-4"/> PHOTA administration</p><h1 className="mt-2 text-3xl font-semibold text-white">Verification and security center</h1><p className="mt-2 text-slate-400">Review hospital applications and public accounts paused by automated protection.</p></div><div className="flex gap-2"><Link href="/profile" className="secondary-btn">Admin profile</Link><LogoutButton/></div></header><AdminHospitalList initialHospitals={hospitals}/><AdminPublicUserList initialUsers={blockedUsers}/></div></main>;
}
