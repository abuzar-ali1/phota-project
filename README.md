...
- Authenticated requester/donor matches with direct WhatsApp click-to-chat contact.
...
`RESEND_API_KEY` and `EMAIL_FROM` are required to deliver verification codes. Public matches use WhatsApp click-to-chat links and do not require an AI provider credential.
...
- `lib/abuse.ts` and `lib/security.ts` — throttling, account blocking, and request protections.
