...
export async function deleteHospital(id:string){await ensureSchema();const sql=database();const rows=await withRetry(()=>sql`DELETE FROM hospitals WHERE id=${id} AND role='hospital' RETURNING hospital_name`);return rows[0]?string(rows[0].hospital_name):null;}
