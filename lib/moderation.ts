...
function providerFailure(status: number): ModerationDecision {
  if (status === 429) {
    return {
      allowed: false,
      reason: "The AI safety service is temporarily rate-limited. Wait at least one minute before trying again.",
  ...
  if (status === 401 || status === 403) {
    return {
      allowed: false,
      reason: "The AI safety service needs administrator attention. Message delivery is paused.",
      source: "local-fallback",
      httpStatus: 503,
    };
  }
  return {
    allowed: false,
    reason: "AI safety moderation is temporarily unavailable. Message delivery is paused.",
    source: "local-fallback",
    httpStatus: 503,
  };
}
...
export async function moderateMessage(raw: string): Promise<ModerationDecision> {
  ...
  for (const rule of localRules) {
    if (rule.pattern.test(content)) return { allowed: false, reason: rule.reason, source: "local" };
  }
  ...
  if (!apiKey) {
    return {
      allowed: false,
      reason: "AI safety moderation is not configured. Message delivery is paused.",
      source: "local-fallback",
      httpStatus: 503,
    };
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error("OpenAI moderation request rejected", {
    ...
    if (result.flagged) {
      ...
      return {
        allowed: false,
        reason: `The safety moderator blocked this message${categories.length ? `: ${categories.join(", ")}` : "."}`,
        source: "openai",
      };
    }
    return { allowed: true, reason: null, source: "openai" };
  } catch (error) {
    console.error("AI moderation unavailable; message delivery remains paused", error);
    return {
      allowed: false,
      reason: "AI safety moderation is temporarily unavailable. Message delivery is paused.",
      source: "local-fallback",
