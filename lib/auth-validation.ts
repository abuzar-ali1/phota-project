...
export const HOSPITAL_LICENSE_MIN_LENGTH = 5;
export const HOSPITAL_LICENSE_MAX_LENGTH = 40;
export const HOSPITAL_LICENSE_HTML_PATTERN = "[A-Za-z0-9][A-Za-z0-9/&.\\-]{3,38}[A-Za-z0-9]";
const HOSPITAL_LICENSE_PATTERN = /^[A-Z0-9][A-Z0-9/&.-]{3,38}[A-Z0-9]$/;

export function normalizeHospitalLicense(value: unknown) {
  return String(value || "").trim().toUpperCase().replace(/\s+/g, "");
}

export function validHospitalLicense(value: string) {
  return value.length >= HOSPITAL_LICENSE_MIN_LENGTH
    && value.length <= HOSPITAL_LICENSE_MAX_LENGTH
    && HOSPITAL_LICENSE_PATTERN.test(value);
}

export function parseSignup(body: Record<string, unknown>): HospitalSignupInput | null {
  const licenseNumber = normalizeHospitalLicense(body.licenseNumber);
  const input = {
    hospitalName: clean(body.hospitalName), hospitalType: clean(body.hospitalType, 50), licenseNumber,
  ...
  if (!input.hospitalName || !input.hospitalType || !validHospitalLicense(input.licenseNumber) || !input.registrationAuthority || !validEmail(input.officialEmail) || !input.phone || !input.address || !input.city || !input.adminName || !input.adminTitle || !validPassword(input.password)) return null;
  return input;
}
